...
const extractErrors = (text, lang) => {
    if (lang === "cpp") {
        const lastExecution = text.lastIndexOf("Executing task");
        text = text.substr(lastExecution);
        return text.match(/\d+:\d+: error: .*/gi);
    }
    else if (lang === "typescript") {
        const lastExecution = text.lastIndexOf("tsc");
        text = text.substr(lastExecution);
        return text.match(/:\d+:\d+ - error .*/gi);
    }
    else if (lang === "python") {
        const lastExecution = text.lastIndexOf("python.exe");
        text = text.substr(lastExecution);
        return text.match(/\n[a-zA-Z]*Error.*/gi);
    }
    return null;
};
const toStackOverflowTag = (lang) => {
    if (lang === 'cpp') {
        return 'c++';
    }
    else if (lang === "typescript") {
...
const fetchAndFilterSearchResults = (errorWords, currentPanel, lang, minWordLimit, maxWordLimit) => {
};
const fetchSearchResults = (errorWords, currentPanel, lang, wordLimit, context) => {
...
const captureTerminal = (currentPanel, context) => {
...
const getOrCreatePanel = (currentPanel, context) => {
    if (!currentPanel) {
        currentPanel = vscode.window.createWebviewPanel('searchResults', 'Search Results', vscode.ViewColumn.One, {
            enableScripts: true
        });
